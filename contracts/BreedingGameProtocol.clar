...
(define-public (breed-creatures (parent1-id uint) (parent2-id uint))
  (let (
    ...
    (parent1-gen (get generation parent1-data))
    (parent2-gen (get generation parent2-data))
    (max-generation (if (> parent1-gen parent2-gen) parent1-gen parent2-gen))
    (offspring-genes (+ 
      (/ (+ (get genes parent1-data) (get genes parent2-data)) u2)
      (mod (+ current-block new-creature-id) u1000000)))
...
(define-read-only (can-breed (creature-id uint))
  (match (map-get? creatures creature-id)
    creature-data 
      (ok (>= stacks-block-height (+ (get last-breed-time creature-data) breeding-cooldown-blocks)))
    (ok false)))
...
(define-public (create-genesis-creature (recipient principal) (genes uint))
  ...
  (begin
    ...
    (map-set creatures new-creature-id {
      genes: genes,
      generation: u0,
      birth-time: stacks-block-height,
      last-breed-time: u0,
      parent1: none,
      parent2: none
    })
